...
public enum Locale {
	...
	public String format(Object... objects) {
		return new MessageFormat(ChatColor.translateAlternateColorCodes('&',
				Tails.get().getMainConfig().getString(path))).format(objects);
	}

	public List<String> formatLines(Object... objects) {
		List<String> lines = new ArrayList<>();

		if (Tails.get().getMainConfig().get(path) instanceof String) {
			lines.add(new MessageFormat(ChatColor.translateAlternateColorCodes('&',
					Tails.get().getMainConfig().getString(path))).format(objects));
		} else {
			for (String string : Tails.get().getMainConfig().getStringList(path)) {
